...
char *make_block(int block_size) {
  ...
  int s = block_size;
  char * ret = (char *) malloc(sizeof(char) * s);
  for (int i = 0; i < s; i++) {
    ret[i] = '0';
  }
...
Line *make_lines(int line_count, int block_size) {
  ...
  Line * ret = (Line *) malloc(sizeof(Line) * line_count);
  for (int i = 0; i < line_count; i++) {
    ret[i].valid = '1';
    ret[i].tag = 1;
    ret[i].accessed = make_block(block_size);
    ret[i].block_size = block_size; 
  }
...
Set *make_sets(int set_count, int line_count, int block_size) {
  ...
  //
  Set * ret = (Set *) malloc(sizeof(Set) * set_count);
  for (int i = 0; i < set_count; i++) {
    ret[i].lines = make_lines(line_count, block_size);
    ret[i].line_count = line_count;
    ret[i].lru_queue = NULL;  
  }
